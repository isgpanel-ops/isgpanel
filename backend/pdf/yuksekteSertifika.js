...
const kurumFirma = safe(
  ...
  payload?.kurumsal?.unvan ||
  payload?.kurumsal?.unvanAdi ||
  payload?.kurumsal?.title ||
  payload?.kurumsal?.companyName ||
  ""
);
  ...
  const egitimTarih = formatDateTR(
  ...
  const egitimSure = safe(
    payload?.egitim?.genelSaat ||
      payload?.egitim?.saat ||
      payload?.egitimSure ||
      ""
  );

  const belgeNo = makeBelgeNo(payload);
...
async function createSertifikaPdf(payload, req = null) {
  ...
  try {
    ...
    pdfBuffer = await createTrainingFallbackPdf(payload, {
      title: "YÜKSEKTE ÇALIŞMA EĞİTİM SERTİFİKASI",
      landscape: true,
    });
  }
...
module.exports = {
  createSertifikaPdf,
  createYuksekteSertifikaPdf: createSertifikaPdf,
};
