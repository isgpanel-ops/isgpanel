...
// Bireyseldekiyle aynı türler :contentReference[oaicite:5]{index=5}
...
// kaldırıldı → badge artık direkt type kullanıyor
...
// TR + Windows yasaklı karakter sanitize (bireyselle aynı yaklaşım) :contentReference[oaicite:6]{index=6}
...
// Backend doc normalize: UI aynı kalsın
const normalizeDoc = (d) => {
  const id = d?._id || d?.id || "";
  const firmaId = d?.firmaId || d?.firma?._id || d?.firma?.id || "";
  const firmaAdi = d?.firmaAdi || d?.firma?.firmaAdi || d?.firma?.name || "";
  const fileUrl = d?.fileUrl || d?.url || d?.pdfUrl || d?.downloadUrl || "";

  return {
    id: String(id),
    firmaId: String(firmaId),
    firmaAdi,
    tarih: d?.tarih || d?.createdAt || "",
    createdAt: d?.createdAt || "",
    durum: d?.durum || d?.status || "Hazır",
    belgeTuru: d?.belgeTuru || d?.tur || d?.type || "",
   baslik: (d?.baslik || d?.title || d?.belgeAdi || "")
  .replace(/^[^-–—]+[-–—]\s*/, "") // baştaki firma adı + tireyi sil
  .trim() || "Belge",
    hazirlayan: d?.hazirlayan || d?.createdBy || d?.preparedBy || "",
    fileUrl,
    url: fileUrl,
    _raw: d,
  };
    ...
    >
      <div className="w-full max-w-4xl rounded-2xl bg-white shadow-xl overflow-hidden">
        <div className="flex items-center justify-between px-4 py-2 border-b">
          ...
          {/* bireyseldeki gibi: sadece Kapat :contentReference[oaicite:7]{index=7} */}
  ...
  for (const firmId of firmIds) {
    try {
      const res = await axios.get(`${API_BASE}/api/documents`, {
        headers,
        params: { firmaId: firmId, category, scope: DOC_SCOPE }, // ticari admin sözleşmesi :contentReference[oaicite:8]{index=8}
      });
...
async function updateDocumentStatus(docId, durum) {
  ...
  try {
    await axios.patch(`${API_BASE}/api/documents/${docId}`, payload, { headers });
    return true;
  } catch (_) {}

  // 2) PUT /api/documents/:id
  try {
    await axios.put(`${API_BASE}/api/documents/${docId}`, payload, { headers });
    return true;
  } catch (_) {}

  // 3) Admin fallback
  await axios.patch(`${API_BASE}/api/admin/documents/${docId}`, payload, { headers });
  return true;
}
...
async function deleteDocumentById(docId) {
  ...
  try {
    await axios.delete(`${API_BASE}/api/documents/${docId}`, { headers });
    return true;
  } catch (_) {}

  await axios.delete(`${API_BASE}/api/admin/documents/${docId}`, { headers });
  return true;
}

export default function TicariAcilDurumBelgeleri() {
  const location = useLocation();

  // TicariBelgeler’deki gibi ?u / localStorage :contentReference[oaicite:9]{index=9}
  ...
  const [firms, setFirms] = useState([]); // {id,firmaAdi}
  const [selectedFirmId, setSelectedFirmId] = useState("Tüm");

  const [docs, setDocs] = useState([]);
  const [loading, setLoading] = useState(false);

  // filtreler (bireyseldekiyle aynı yapı) 
  ...
  // ConfirmModal state (bireyseldeki pattern) :contentReference[oaicite:11]{index=11}
  ...
  useEffect(() => {
    ...
    if (u) {
      ...
      const cached = localStorage.getItem(LS_ADMIN_USER_KEY) || "all";
      setSelectedUserId(cached);
    }
  }, [location.search]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const raw = await fetchDocsForFirmIds(firmIds, "acil");
        if (!alive) return;
        setDocs(raw.map(normalizeDoc));
      } catch (e) {
        console.error("fetchDocsForFirmIds error:", e);
        if (alive) setDocs([]);
      } finally {
        if (alive) setLoading(false);
      }
    ...
    return docs
      .filter((d) => {
        ...
        if (search.trim()) {
          ...
          const hay = `${d.firmaAdi || ""} ${t} ${getBadgeLabel(t)} ${toDisplayDate(
            d.tarih || d.createdAt
    ...
    openConfirm({
      ...
      onConfirm: async () => {
        try {
          ...
          console.error(e);
          openInfo("Hata", "Belge arşivlenemedi. (Backend endpoint kontrol edin)");
        }
      },
    });
  };
    ...
    openConfirm({
      ...
      onConfirm: async () => {
        try {
          ...
          console.error(e);
          openInfo("Hata", "Belge geri alınamadı. (Backend endpoint kontrol edin)");
        }
      },
    });
  };
  ...
  const handleSil = (doc) => {
    if (doc.durum === "Arşivde") return; // bireyseldeki kilit davranışı :contentReference[oaicite:12]{index=12}
    ...
    openConfirm({
      ...
      onConfirm: async () => {
        try {
          ...
          console.error(e);
          openInfo("Hata", "Belge silinemedi. (Backend endpoint kontrol edin)");
        }
      },
    });
  };
  ...
  return (
    <div className="bg-white rounded-xl border p-4">
      <div className="mb-3">
        <div className="text-sm font-semibold text-[#042f4b]">Acil Durum Belgeleri</div>
        <div className="text-xs text-gray-500">
          Acil durum planı ve acil ekip belgeleri burada listelenir.
        </div>
      </div>
          ...
          <table className="min-w-full text-[12px]">
            ...
            <tbody className="divide-y divide-gray-100">
              ...
              {!loading && filteredDocs.length === 0 && (
                <tr>
                  <td colSpan={5} className="px-3 py-6 text-center text-gray-400">
                    Kayıtlı acil durum belgesi bulunamadı.
                  </td>
                </tr>
              )}
