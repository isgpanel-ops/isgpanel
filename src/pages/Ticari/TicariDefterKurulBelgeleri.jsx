...
const CATEGORY = "defter-kurul";
const LS_ADMIN_USER_KEY = "isgpanel:adminSelectedUser";

const BELGE_TURLERI = ["Defter Nüshaları", "Kurul Nüshaları"];
...
const getPreparedBy = (doc) => {
  ...
  const userObj =
    doc?.user && typeof doc.user === "object" ? doc.user : null;

  const rawObj =
    doc?._raw && typeof doc._raw === "object" ? doc._raw : null;
  ...
  const v =
    ...
    userObj?.adSoyad ||
    rawObj?.hazirlayan ||
    rawObj?.hazirlayanAdSoyad ||
    rawObj?.preparedBy ||
    rawObj?.preparedByName ||
    rawObj?.createdByName ||
    rawObj?.createdBy?.name ||
    rawObj?.createdBy?.fullName ||
    "";
...
const inferBelgeTuru = (doc) => {
  ...
  if (raw.includes("kurul")) return "Kurul Nüshaları";
  if (raw.includes("defter")) return "Defter Nüshaları";

  const title = String(doc?.baslik || doc?.title || doc?.belgeAdi || "").toLowerCase();
  if (title.includes("kurul")) return "Kurul Nüshaları";
  if (title.includes("defter")) return "Defter Nüshaları";

  return doc?.belgeTuru || "Defter & Kurul";
};

const TYPE_BADGE_CLASS = (type) => {
  const t = String(type || "").toLowerCase();
  if (t.includes("defter")) return "bg-blue-100 text-blue-700";
  if (t.includes("kurul")) return "bg-violet-100 text-violet-700";
...
export default function TicariDefterKurulBelgeleri() {
  ...
  return (
    <div className="bg-white rounded-xl border p-4">
      <div className="mb-3">
        <div className="text-sm font-semibold text-[#042f4b]">Defter & Kurul Belgeleri</div>
        <div className="text-xs text-gray-500">
          Defter nüshaları ve kurul nüshaları burada listelenir.
        </div>
      </div>
          ...
          <table className="min-w-full text-[12px]">
            ...
            <tbody className="divide-y divide-gray-100">
              ...
              {!loading && filteredDocs.length === 0 && (
                <tr>
                  <td colSpan={5} className="px-3 py-6 text-center text-gray-400">
                    Kayıtlı defter/kurul belgesi bulunamadı.
                  </td>
                </tr>
              )}
