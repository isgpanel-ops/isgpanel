...
useEffect(() => {
  ...
  const exportPdf = async (mode = "save") => {
    ...
    doc.setProperties({
      title: `${safeFirma} - Risk Değerlendirmesi`,
      subject: "Risk Değerlendirmesi",
      creator: "ISG Panel",
    });
    doc.setFont("helvetica", "bold");
    doc.setFontSize(9);
    doc.text(String(firmaAdi || safeFirma).toLocaleUpperCase("tr-TR"), pageW / 2, 22, { align: "center" });
    doc.setFontSize(12);
    doc.text("RISK DEGERLENDIRMESI", pageW / 2, 39, { align: "center" });
    doc.setFont("helvetica", "normal");
    doc.setFontSize(7);
    [
      `Hazirlama: ${hazirlamaTarihi || belgeTarihTr}`,
      `Gecerlilik: ${gecerlilikTarihi || "-"}`,
      `Revizyon: ${revNo || "-"} / ${revDate ? new Date(revDate).toLocaleDateString("tr-TR") : "-"}`,
    ].forEach((text, idx) => doc.text(text, 18, 57 + idx * 10));
    [
      `Tehlike sinifi: ${tehlikeSinifi || "-"}`,
      `SGK Sicil No: ${sgkSicilNo || "-"}`,
      `Adres: ${selectedFirm?.adres || "-"}`,
    ].forEach((text, idx) => doc.text(text, pageW - 18, 57 + idx * 10, { align: "right", maxWidth: 330 }));

    const headers = [...HEADERS_LEFT, ...HEADERS_RIGHT];
    const body = rows.map((r) => [
      ...
      asText(r.riskTanimi),
      asText(r.onlemler),
      asText(r.mevcutDurum),
      asText(r.sorumlu),
      asText(r.termin),
      asText(r.olasilikSon),
      asText(r.siddetSon),
      asText(r.rdsSon),
    ]);
    ...
    doc.autoTable({
      ...
      body,
      startY: 90,
      margin: { left: 12, right: 12, top: 88, bottom: 24 },
      tableWidth: pageW - 24,
      theme: "grid",
      styles: {
        font: "helvetica",
      ...
      columnStyles: {
        ...
        17: { cellWidth: 24, halign: "center" },
      },
      didParseCell: (data) => {
        if (data.section !== "body") return;
        if (data.column.index === 9 || data.column.index === 17) {
      ...
      didDrawPage: () => {
        ...
        doc.text(`Sayfa ${pageNumber}`, pageW - 18, pageH - 10, { align: "right" });
      },
    });
