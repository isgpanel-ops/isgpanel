...
export default function DenetimGoruntule() {
  ...
  const loadPackage = async (candidatePassword = "") => {
    ...
    setError("");
    try {
      const query = candidatePassword ? `?password=${encodeURIComponent(candidatePassword)}` : "";
      const response = await fetch(`${AUDIT_API_BASE}/audit-packages/public/${encodeURIComponent(publicToken)}${query}`);
  ...
  if (loading) return <PublicState title="Belge paylaşımı hazırlanıyor..." />;
    ...
    return (
      <main className="audit-state-page">
        <section className="audit-state-card">
          ...
          <p>Bu belge paylaşımını görüntülemek için paylaşım şifresini giriniz.</p>
          <form onSubmit={(event) => { event.preventDefault(); loadPackage(passwordInput); }}>
            ...
            <button type="submit" disabled={!passwordInput}>Belgeleri Görüntüle</button>
          </form>
        </section>
        <PortalStyles />
      </main>
    ...
    <main className="audit-public-page">
      <header className="audit-header">
        <div className="audit-brand">
          {pkg.organizationLogoUrl ? <img className="audit-brand-logo" src={assetUrl(pkg.organizationLogoUrl)} alt={pkg.organizationName || "Kurumsal logo"} /> : <ShieldCheck size={30} />}
          <div><strong>{pkg.organizationName || "İSG PANEL"}</strong><span>BELGE PAYLAŞIMI</span></div>
        </div>
        <div className="audit-file-number">Dosya No <strong>{pkg.packageNumber}</strong></div>
      </header>

      <section className="audit-hero">
        <span className="audit-badge">DİJİTAL İSG BELGE PAYLAŞIMI</span>
      ...
      <div className="audit-layout">
        <aside className="audit-sidebar">
          <button className={activeCategory === "summary" ? "active" : ""} onClick={() => setActiveCategory("summary")}><ShieldCheck size={17} /> Belge Özeti</button>
        ...
        <section className="audit-content">
          {activeCategory === "summary" ? (
            <><div className="audit-section-title"><div><h2>Belge Özeti</h2><p>Paylaşılan belge kategorileri ve belge sayıları.</p></div></div><div className="audit-category-grid">{categories.map((category) => { const key = category.id || category.category || category.name; return <button key={key} onClick={() => setActiveCategory(key)}><FolderOpen size={22} /><span>{category.name || category.label || category.category}</span><strong>{category.count ?? category.documents?.length ?? 0} belge</strong></button>; })}</div></>
          ) : (
