...
function getTypeLabel(doc) {
  ...
  if (sub.includes("defter")) return "Defter Nüshaları";
  if (sub.includes("kurul")) return "Kurul Nüshaları";
...
function badgeClass(label) {
  ...
  if (l.includes("defter")) return "bg-blue-100 text-blue-700";
  if (l.includes("kurul")) return "bg-violet-100 text-violet-700";
...
export default function DefterKurulBelgeleri() {
  ...
  async function fetchDocs() {
    try {
      ...
      setDocs(
        list.filter(
          (d) => String(d?.category || "").toLowerCase() === "defter-kurul"
        )
      );
    } catch (e) {
  ...
  async function handleIndir(doc) {
    try {
      ...
      if (!url) throw new Error("Dosya URL bulunamadı.");
...
async function handleGeriAl(doc) {
  ...
  return (
    <CardBox className="flex flex-col gap-4">
      <SectionTitle
        title="Defter & Kurul Belgeleri"
        subtitle={`${selectedFirm.firmaAdi || selectedFirm.name || ""} firmasına ait defter ve kurul nüshaları burada listelenir.`}
      />
